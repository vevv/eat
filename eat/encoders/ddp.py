...
class Encoder(DeeEncoder):
    ...
    def _configure(self, *args, **kwargs):
        ...
        # Configure encoder
        bitrate = kwargs['bitrate'] or self._get_default_bitrate(kwargs['channels'])
        mix = self._get_mix(kwargs['channels'])
        if kwargs.get('remix', False):
            self.logger.warning('Remixing audio to %s', mix)

        if mix == '7.1':  # Upmix to 7.1 requires different encoder mode
            config.filter.audio.pcm_to_ddp.encoder_mode = 'ddp71'
            mix = 'off'
        elif bitrate > 1280:  # Use bluray profile for bitrates >1280 kbps
